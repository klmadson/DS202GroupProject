...
My focus for this research question was to identify the coorelation between resources such as computer and internet and if they have a significant impact on student scores. Following the same, I was able to draw a conclusion as follows:
...
```{r 1st box plot, echo=FALSE, fig.cap="In this box plot of father education by math score, I can see that with a 3A education for the father, the child's math scores are visibly higher. While there are outliers on both the high and low spectrum of scores, there could be other factors like internet access effecting this. This graph reaffirms the idea that with a higher education for parents, the child's score will be higher.", message = FALSE}

USA_students %>%
  ggplot(aes(x=father_educ,y=math,color=father_educ)) + geom_boxplot()
```

```{r 2nd box plot, echo=FALSE, fig.cap="In this box plot of father education by science score, I can see that with a 3A education for the father, the child's science scores are visibly higher. While there are outliers on both the high and low spectrum of scores similar to math scores, there could be other factors like internet access effecting this. This graph reaffirms the idea that with a higher education for parents, the child's score will be higher.", message = FALSE}

USA_students %>%
  ggplot(aes(x=father_educ,y=science,color=father_educ)) + geom_boxplot()
```

```{r 3rd box plot, echo=FALSE, fig.cap="In this box plot of father education by reading score, I can see that with a 3A education for the father, the child's reading scores are visibly higher. While there are outliers on both the high and low spectrum of scores similar to that of math and science scores, there could be other factors like internet access effecting this. This graph reaffirms the idea that with a higher education for parents, the child's score will be higher.", message = FALSE}

USA_students %>%
  ggplot(aes(x=father_educ,y=read,color=father_educ)) + geom_boxplot()
```
...
When it comes to resources and their correlation with subject scores, I was able to identify and conclude that the usage of resources such as the computer or internet is highly beneficial in deciding whether a student does good or bad in their subjects. According to my research, having only one resource can result into imporved scores however, might not be the best solution for students who want to perform well in class. 
...
When looking at gender, it appears as though the distribution of scores is not uniform across subjects. Female students outperform men by a significant margin in reading, while male students students have a slightly larger mean score than female students in math. I believe that many of these differences in scores represent cultural attitudes surrounding these subjects in the contemporary United States. Women tend to be raised with an increased emphasis on communication skills relative to men. It is also possible that girls generally enjoy reading more books from an early age than do boys, which would increase their performance on these tests. On the other hand, men are often celebrated more for their technical skills than their cultural knowledge or communication skills (at least in the United States). Furthermore, the unfortunate notion of STEM subjects being a "boy's club" still persists in contemporary Western culture, though this is beginning to change. I predict that as culture changes to be more inclusive toward women interested in Math and to celebrate mathematical ability in women to a greater degree, math scores will increase for female students. Furthermore, I believe a similar principle holds true for male students with respect for reading: boys must be celebrated for their ability to comprehend complex texts and to use language effectively. It would be interesting to compare scores with other years (as well as the upcoming round of tests) to determine if scores are indeed moving in a direction of becoming more balanced.

As we've predicted, performance in all three subjects measured correlate very highly with one another. This has potential implications for the validity of the idea of highly specified "talents" being the main driver of performance in a given subject. This goes hand in hand with the common notion of "being a math person"; that being good at math necessarily means that you are "worse" in reading. Our findings, however, find a high correlation between math and reading scores. This suggest that there is a common factor underlying performance across all three domains. I would predict that "hard work" and the presence of good study habits plays a significant role in this.
